...
int ComponentCallback(defrCallbackType_e typ, defiComponent* comp,
  ...
  if (comp->isPlaced() || comp->isFixed()) {
    ...
    // const double width = (height == 3)?
    //                      (cell.width() + 2 * Site::width()):
    //                      cell.width();
  ...
  while (getline(def, line)) {
    if (component == false) {
      ...
      if (line.substr(0, 10) == "COMPONENTS") {
        component = true;
        cout << "OutputDef: component true" << endl;
      }
    } else {
      out << line << endl;
      if (line == "END COMPONENTS") {
        component = false;
        cout << "OutputDef: component false" << endl;
        break;
      }
      getline(def, line);
      const Instance& instance = database_.instances_[k];
      if (line.substr(4, 6) == "PLACED") {
        out << "		+ PLACED ( " << instance.position().x() << " "
            << instance.position().y() << " "
            << ") " << instance.orientation() << " ;" << endl;
      } else {
        cout << "OutputDef: PLACED else" << endl;
        cout << "OutputDef: " << line.substr(8, 6) << endl;
        out << line << endl;
      }
      k++;
    }
