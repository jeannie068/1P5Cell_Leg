...
void Database::Summary(ostream& os) const {
  ...
  double total_instance_area = 0.0;
  double total_1p5CH_instance_area = 0.0;
  for (int i = 0; i < instances_.size(); ++i) {
    const Instance& instance = instances_[i];
    const double width = instance.width();
    const double height = instance.height();
    total_instance_area += width * height;
    if (instance.num_sub_instances() == 3) {
        total_1p5CH_instance_area += width * height;
    }
  }
  os << "Area of Instances: " << total_instance_area << endl;
  os << "  Area of 1.5CH Instances: " << total_1p5CH_instance_area << endl;
  os << "  Area Ratio of 1.5CH Instances: " << double(total_1p5CH_instance_area/total_instance_area) << endl;
  os << "  # of Fixed: " << fixed_instance_ids_.size() << endl;
  for (int i = 0; i < instance_ids_by_row_height_.size(); ++i) {
    os << "  # of " << i + 1
       << "-Row-Height: " << instance_ids_by_row_height_[i].size() << endl;
...
void Database::Plot(ostream& os, const string& title) const {
  ...
  LineStyleId die_rect_style_id = plotter.add_line_style("lc rgb 'pink' lw 5");
  ...
  LineStyleId row_style_id =
      plotter.add_line_style("lc rgb 'grey' lw 1.5 dt '-'");
  LineStyleId power_rail_style_id =
      plotter.add_line_style("lc rgb 'orange' lw 1.5 dt '-'");
  LineStyleId ground_rail_style_id =
      plotter.add_line_style("lc rgb 'blue' lw 1.5 dt '-'");
  // LineStyleId row_style_id =
  //     plotter.add_line_style("lc rgb 'grey' lw 1.5");
  // LineStyleId power_rail_style_id =
  //     plotter.add_line_style("lc rgb 'orange' lw 1.5");
  // LineStyleId ground_rail_style_id =
  //     plotter.add_line_style("lc rgb 'blue' lw 1.5");
  ...
  LineStyleId three_row_height_instance_style_id = plotter.add_line_style("lc rgb 'red' lw 2");
  // 填充樣式
  LineStyleId two_row_height_fill_style_id = plotter.add_line_style("lc rgb 'green'");
  LineStyleId three_row_height_fill_style_id = plotter.add_line_style("lc rgb 'red'");
  LineStyleId black_border_style_id = plotter.add_line_style("lc rgb 'black' lw 1.5");
  ...
  for (int i = 0; i < instances_.size(); ++i) {
    ...
    if (instance.num_sub_instances() == 2) {
        // 兩行高的 cell
        width_tmp = instance.width();
    } else if (instance.num_sub_instances() == 3) {
        // 三行高的 cell
        width_tmp = instance.width() - 2 * site_width;
    }
    const double width = width_tmp;
    const double height = instance.height();
    ...
    // 判斷 cell 高度並繪製
    if (instance.num_sub_instances() == 2) {
        // 兩行高的 cell - 綠色實心矩形黑色外框
        plotter.add_filled_rect(rect, two_row_height_fill_style_id, black_border_style_id);
    } else if (instance.num_sub_instances() == 3) {
        // 三行高的 cell - 紅色實心矩形黑色外框  
        plotter.add_filled_rect(rect, three_row_height_fill_style_id, black_border_style_id);
    } else {
        // 其他 cell - 只畫外框
        const FenceRegionId fence_region_id = instance.fence_region_id();
        LineStyleId current_style_id = (fence_region_id != UNDEFINED_ID) 
            ? instance_fence_region_style_ids.at(fence_region_id)
            : (instance.forbidden_cell ? forbidden_instance_style_id : instance_style_id);
        plotter.add_rect(rect, current_style_id);
    }
    ...
    // 決定方向標記的樣式
    LineStyleId mark_style_id = black_border_style_id; // 預設使用黑色
    if (instance.num_sub_instances() < 2) { // 單行高的 cell 使用原本的樣式
        const FenceRegionId fence_region_id = instance.fence_region_id();
        mark_style_id = (fence_region_id != UNDEFINED_ID) 
            ? instance_fence_region_style_ids.at(fence_region_id)
            : (instance.forbidden_cell ? forbidden_instance_style_id : instance_style_id);
    }
    ...
    switch (orientation) {
        ...
        case Orientation::S: {
            plotter.add_line(
                Line(Point(instance.position().x() + span, instance.position().y() + height),
                     Point(instance.position().x() + width, instance.position().y() + height - span)),
                mark_style_id);
            break;
        }
        case Orientation::FN: {
            plotter.add_line(
                Line(Point(instance.position().x() + span, instance.position().y()),
                     Point(instance.position().x() + width, instance.position().y() + span)),
                mark_style_id);
            break;
        }
        case Orientation::FS: {
            plotter.add_line(
                Line(Point(instance.position().x() + span, instance.position().y() + height),
                     Point(instance.position().x(), instance.position().y() + height - span)),
                mark_style_id);
            break;
        }
        default: { 
...
long long Database::ComputeTotalHpwl() const {
  long long total_hpwl = 0;

  for (int i = 0; i < nets_.size(); ++i) {
    const Net& net = nets_[i];
    
    // Count total number of pins in this net
    int total_pins = net.num_pins() + net.num_instance_pins();
    if (total_pins <= 1) {
      continue;
    }
    ...
    // Process I/O pins (directly stored position)
    for (int j = 0; j < net.num_pins(); ++j) {
      const PinId pin_id = net.pin_id(j);
      const Pin& pin = pins_.at(pin_id);
      
      // I/O pins directly store their position
      int x = pin.position().x();
      int y = pin.position().y();
      
      // If pin has ports, use the center of the first port
      if (pin.num_ports() > 0) {
        const Port& port = pin.port(0);
        const Rect& rect = port.rect();
        x += (rect.min_corner().x() + rect.max_corner().x()) / 2;
        y += (rect.min_corner().y() + rect.max_corner().y()) / 2;
      }
    ...
    // Process instance pins (need to calculate from instance position)
    for (int j = 0; j < net.num_instance_pins(); ++j) {
      const InstancePinId instance_pin_id = net.instance_pin_id(j);
      const InstancePin& instance_pin = instance_pins_.at(instance_pin_id);
      
      // Get the instance
      const InstanceId instance_id = instance_pin.instance_id();
      const Instance& instance = instances_.at(instance_id);
      
      // Get the cell pin
      const CellPinId cell_pin_id = instance_pin.cell_pin_id();
      const CellPin& cell_pin = cell_pins_.at(cell_pin_id);
      
      // Get instance position
      int inst_x = instance.position().x();
      int inst_y = instance.position().y();
      
      // Calculate pin center relative to instance
      int pin_x = inst_x;
      int pin_y = inst_y;
      
      // If cell pin has ports, use the center of the first port
      if (cell_pin.num_ports() > 0) {
        const CellPinPort& port = cell_pin.port(0);
        const Rect& rect = port.rect();
        
        // Add the center of the port rectangle relative to the instance origin
        pin_x += (rect.min_corner().x() + rect.max_corner().x()) / 2;
        pin_y += (rect.min_corner().y() + rect.max_corner().y()) / 2;
      }
      // Alternatively, if instance pin has ports, use those
      else if (instance_pin.num_ports() > 0) {
        const InstancePinPort& port = instance_pin.port(0);
        const Rect& rect = port.rect();
    ...
    if (lx != INT_MAX && hx != INT_MIN) {
      total_hpwl += (hx - lx) + (hy - ly);
    }
  }
