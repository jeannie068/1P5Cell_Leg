...
int main(int argc, char* argv[]) {
  ...
  /* database.Print(); */
  database.Summary();
  const long long gp_hpwl = database.ComputeTotalHpwl();
  ...
  const long long total_hpwl = database.ComputeTotalHpwl();
  const double gp_hpwl_in_m = gp_hpwl / (database.def_unit() * 1000.0 * 1000.0);
  const double dp_hpwl_in_m = total_hpwl / (database.def_unit() * 1000.0 * 1000.0);
  const double delta_hpwl_in_m = (total_hpwl - gp_hpwl) / (database.def_unit() * 1000.0 * 1000.0);

  cout << "Total displacement after detailed placement: " << total_displacement << endl;
  cout << "Total HPWL after global placement (in m): " << gp_hpwl_in_m << endl;
  // cout << "Total HPWL after detailed placement: " << total_hpwl << endl;
  cout << "Delta HPWL (in m): " << delta_hpwl_in_m << endl;
  cout << "HPWL after detailed placement (in m): " << dp_hpwl_in_m << endl;
  ...
  if (arguments.count("plg") == 1) {
    ...
    database.Plot(plot,
                  "After Detailed Placement (in m): " + to_string(dp_hpwl_in_m));
  }
