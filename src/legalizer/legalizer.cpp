...
void Legalizer::PreMmsim() {
  ...
  cout << "draw gp result..." << endl;
  ofstream plot_gp("plot_tmp/" + database_.design_name() + "_gp.plt"); 
  database_.Plot( 
      plot_gp, 
      "GP Result: " + to_string(database_.ComputeTotalDisplacement())); 

  cout << "Align instances to rows..." << endl;

  AlignInstancesToRows();

  ofstream plot_al("plot_tmp/" + database_.design_name() + "_al.plt");
  database_.Plot(
      plot_al,
...
void Legalizer::Mmsim() {
  ...
  // cout << "End Output LCP Parameters: " << endl << endl;
  ...
  cout << "Finish MMSIM after " << k << " iterations." << endl;

  ofstream plot_mmsim("plot_tmp/" + database_.design_name() + "_mmsim.plt");
  database_.Plot(
      plot_mmsim,
      "MMSIM Result: " + to_string(database_.ComputeTotalDisplacement()));
}
...
void Legalizer::PostMmsim() {
  ...
  ofstream plot_al2site("plot_tmp/" + database_.design_name() + "_al2site.plt");
  database_.Plot(
      plot_al2site,
      "Align To Site Result: dis = " + to_string(database_.ComputeTotalDisplacement()));

  // cout << "Fix instances boundary..." << endl;
  // FixInstancesBoundary();
  // ofstream plot_fb("plot_tmp/" + database_.design_name() + "_fb.plt");
  // database_.Plot(
  //     plot_fb,
  //     "Align Result: " + to_string(database_.ComputeTotalDisplacement()));
  ...
  ofstream plot_allo("plot_tmp/" + database_.design_name() + "_allo.plt");
  database_.Plot(
      plot_allo,
      "Allocate Illegal Instances Result: HPWL = " + to_string(database_.ComputeTotalHpwl()));

  // cout << "Resolve illegal instances..." << endl;
  // ResolveIllegalInstances();

  // ofstream plot_resolve("plot_tmp/" + database_.design_name() + "_resolve.plt");
  // database_.Plot(
  //     plot_resolve,
  //     "Resolve illegal instances Result: HPWL = " + to_string(database_.ComputeTotalHpwl()));
  ...
  if (IsResultLegal()) {
    ...
    ofstream plot_check_postMmsim("plot_tmp/" + database_.design_name() + "_check_postMmsim.plt");
    database_.Plot(
      plot_check_postMmsim,
      "Check PostMmsim Result: " + to_string(database_.ComputeTotalDisplacement()));
  } else {
    cout << endl;
...
void Legalizer::AlignInstancesToRows() {
  ...
  for (int i = 0; i < database_.num_instances(); ++i) {
    ...
    // if (current_instance_row_height == 3) {
    //   // 將 nearest_row_id 調整到最近的 3 的倍數
    //   int remainder = nearest_row_id % 3;
    //   if (remainder == 1) {
    //     best_row_id = RowId(nearest_row_id - 1);  // 往下調整到 3 的倍數
    //   } else if (remainder == 2) {
    //     best_row_id = RowId(nearest_row_id + 1);  // 往上調整到 3 的倍數
    //   }
    // }
...
*/
      ...
      const double overlap_weight = 10.0;
    ...
    if (current_instance.num_sub_instances() == 2) {
      int ground_start = (best_row_id / 4) * 4;  // 對齊到 4n (ground rail)
      int power_start = ((best_row_id + 1) / 4) * 4 - 2;  // 對齊到 4n-1 (power rail 下一行)
      
      // 選擇最接近的有效起始位置
      if (abs(best_row_id - ground_start) <= abs(best_row_id - power_start)) {
          best_row_id = ground_start;
      } else {
          best_row_id = power_start;
      }
    }
...
void Legalizer::AlignInstancesToSites() {
  ...
  cout << "num_sites_limit: " << num_sites_limit << endl;
...
/*
      // 檢查 P/G rail 對齊約束
      const int instance_row_height = current_instance.num_sub_instances();
      
      // *** SR cell 的特殊處理 ***
      if (instance_row_height == 2) { // SR cell
        // 取得底部 sub-instance 的 row id
        const SubInstanceId bottom_sub_id = current_instance.bottom_sub_instance_id();
        const SubInstance& bottom_sub = database_.sub_instance(bottom_sub_id);
        const RowId bottom_row_id(
            static_cast<int>((bottom_sub.position().y() - 
                            database_.die_rect().min_corner().y()) / row_height));
        
        // 檢查是否需要調整
        bool needs_adjustment = false;
        if (current_instance.is_bottom_ground()) {
          // 底部應該在 ground rail (row 0, 4, 8...)
          if (bottom_row_id % 4 != 0) {
            needs_adjustment = true;
          }
        } else {
          // 底部應該在 power rail (row 2, 6, 10...)
          if ((bottom_row_id - 2) % 4 != 0) {
            needs_adjustment = true;
          }
        }
        ...
        if (needs_adjustment) {
          // 找到最近的正確 rail
          RowId new_bottom_row_id = bottom_row_id;
          if (current_instance.is_bottom_ground()) {
            // 對齊到最近的 ground rail
            new_bottom_row_id = (bottom_row_id / 4) * 4;
            if (bottom_row_id % 4 >= 2) {
              new_bottom_row_id += 4;
            }
          } else {
            // 對齊到最近的 power rail
            new_bottom_row_id = ((bottom_row_id + 1) / 4) * 4 - 2;
            if ((bottom_row_id - 2) % 4 >= 2) {
              new_bottom_row_id += 4;
            }
          }
          
          // 更新 instance 位置
          const double new_y = database_.die_rect().min_corner().y() + 
                             new_bottom_row_id * row_height;
          current_instance.set_position(
              Point(current_instance.position().x(), new_y));
...
*/
      ...
      for (int j = 0; j < num_sites_limit; ++j) {
        ...
        if (are_sites_available) {
          const double displacement_limit = 50 * site_width;  // TODO: Tune.
      ...
      // if (best_x == die_right_x) {
        ...
        //   is_trial_successful = true;
        // }
        // cout << " Case: best_x == die_right_x, instance name: " 
        //      << current_instance.name() << endl;
      // }
  ...
  for (int h = database_.max_instance_row_height() - 1; h >= 0; --h) {
    for (int i = 0; i < illegal_instance_ids_by_row_height_.at(h).size(); i++) {
      ...
      for (int j = 0; j < 2 * database_.num_rows(); ++j) {
        ...
        if (current_instance_row_height == 2) {  // SR cell
          // SR cell 不能從 pseudo row 開始
          if (current_row_id % 2 == 1) {
              is_pg_rail_correct = false;
          } else {
              // 檢查是否需要翻轉
              bool will_flip = (current_instance.orientation() != current_row.orientation());
              ...
              // 根據 row 編號判斷 rail type
              bool is_ground_rail = (current_row_id % 4 == 0);
              
              // 檢查是否匹配
              if ((effective_is_bottom_ground && !is_ground_rail) ||
                  (!effective_is_bottom_ground && is_ground_rail)) {
                  is_pg_rail_correct = false;
              }
          }
        }
...
void Legalizer::FixInstancesBoundary() {
  const double die_min_x = database_.die_rect().min_corner().x();
  const double die_max_x = database_.die_rect().max_corner().x();
  
  for (int i = 0; i < database_.num_instances(); ++i) {
    InstanceId instance_id(i);
    Instance& instance = database_.instance(instance_id);
    
    if (instance.is_fixed()) continue;
    
    double new_x = instance.position().x();
    ...
    // 修正超出右邊界  
    else if (new_x + instance.width() > die_max_x) {
      new_x = die_max_x - instance.width();
    }
    
    if (new_x != instance.position().x()) {
      instance.set_position(Point(new_x, instance.position().y()));
      database_.UpdateInstanceSubInstancePositions(instance_id);
    }
  }
}                       

void Legalizer::ResolveIllegalInstances() {
  const double site_width = Site::width();
  const double row_height = Site::height();
    ...
    for (int j = 0; j < instance.num_sub_instances(); ++j) {
      ...
      for (int k = 0; k < instance_site_width; ++k) {
        ...
        if (!site.is_valid() ||
          ...
          cout << "Fixing Wrong site:" << endl;
          cout << "  instance: " << instance.name() << " at "
               << instance.position() << endl;
          TryPlaceInstance(instance_id, instance.position().x(), 100 * site_width);
